...
| --- | --- | --- |
| `views/Contact.vue` | email, Signal link, GitHub, LinkedIn, X, timezone, all contact copy | ✅ config driven |
| `components/Navbar.vue` | display name, handle, GitHub URL | ✅ config driven |
| `components/home/IntroSection.vue` | name, welcome lines, bio | ✅ config driven |
| `components/home/ProblemSolvingSection.vue` | repo URL, description | ✅ config driven |
| `components/home/ProjectsSection.vue` | "Check out my GitHub profile" URL | ✅ config driven |
| `components/home/ExperienceSection.vue` | entire work history (hardcoded array) | ✅ `profile.experience[]` |
| `components/Footer.vue` | copyright owner | ✅ config driven |
| `views/Projects.vue` | GitHub URL | ✅ config driven |
| `utils/index.ts` | anti-bot answer | ✅ moved server side |
| `router/index.ts` | owner name in 10 page titles | ✅ reads `brand.displayName` |
| `forms/BrandForm.vue`, `forms/SocialsForm.vue` | placeholder examples | ✅ generic placeholders |
...
CI enforces this. The `no-personal-data` job greps `src/` and `index.html` for the original owner's
name and handles and fails the build on a hit. As of today that grep returns nothing.
...
## Phase 1: stop leaking identity ✅ done

1. ✅ **Every `fallbackX` constant deleted.** Empty config renders nothing.
2. ✅ **Anti-bot answer made configurable and moved server side.** `configuration.antiBot.question`
...
3. ✅ **Page titles from config.** The router watches `profile.brand.displayName`.
4. ✅ **Neutral `backend/config.example.json`** shipped, matching `defaultConfig`.
5. ✅ **Remaining literals purged** from `Search.vue`, `BrandForm`, `SocialsForm`.
...
13. ✅ **LICENSE** (MIT), **CONTRIBUTING.md**, and issue templates (`bug_report.md`,
    `config_gap.md`).
...
- ✅ **`docs/USAGE.md`**: first run, the config model, every environment variable, the dashboard
  tabs, theming and contrast, the webring, deploying, and backups.
- ✅ **In place editing.** Signed in owners get create, edit and delete controls on the public pages
  themselves, via `components/admin/InlineActions.vue` and `EditorDialog.vue`.
- ✅ **Admin login moved to `/admin`**, with `/admin-signature` kept as a redirect. The footer no
  longer advertises it.
- ✅ **IndieWeb webring** support in the footer, configurable from the Site meta tab and seedable
...
6. **No tests.** CI typechecks, lints and builds, which catches a lot for a project this size, but
   there is no test for the auth flow, the seeding rules or `deepBackfill`. Those three carry the
   most risk of a silent regression.
